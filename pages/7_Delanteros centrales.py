...
@st.cache_data
def load_data():
    return pd.read_excel('data/final_delanterosCentrales_todos_20252026.xlsx')

def main():
    st.set_page_config(page_title="Delanteros centrales", layout="wide")

    df = load_data()

    # 1. Crear columna para el filtro compuesto
    ...
    with col2:
        st.subheader("Delanteros centrales")
        ...
        # 4. Multiselect para el filtro compuesto Pais | Competencia | Año
        ...
        # 6. Crear columna de “Jugador con equipo”
        df['Jugador con equipo'] = df['Player'] + ' (' + df['Team within selected timeframe'] + ')'

        # Mostrar en selectbox sin tildes
        ...
        # 7. Definir parámetros del radar
        datos_radar = [
